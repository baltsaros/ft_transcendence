...
const fetchInvitations = createAsyncThunk('get/fetchInvitations', async(idUser: number) => {
  const invitations = await instance.post('user/getInvitations/', idUser);
  return invitations.data;
})

interface InvitationState {
  invitations: IUserRelation[];
  status: string;
}
...
const invitationSlice = createSlice({
    ...
    initialState,
    reducers: {
        addInvitation: (state, action: PayloadAction<IUserRelation>) => {
            // return [...state, ...action.payload];
            state.invitations.push(action.payload);
        },
        removeInvitation: (state, action: PayloadAction<IUserRelation>) => {
            const invitationToRemove = action.payload;
            state.invitations = state.invitations.filter((invit) => invit.receiverId != invitationToRemove.receiverId
                && invit.senderId !== invitationToRemove.senderId);
        }
    },
